...
class _waterIntakeState extends State<water_intake> {
  ...
  void initState() {
    ...
    Future.delayed(const Duration(milliseconds: 1500), () {
      setState(() {
        _selected =
            List<bool>.generate(waterintake_list.length, (int index) => false);
              ...
              child: GestureDetector(
                onTap: () {
                  showModalBottomSheet(
                    ...
                    waterintake_list = thisReturned.WI_result; // update list
                      ...
                      Future.delayed(const Duration(milliseconds: 2000), () {
                        setState(() {
                          _selected = List<bool>.generate(
                              waterintake_list.length, (int index) => false);
                        });
                      });
                    } else {
              ...
              onPressed: () {
                ...
                for (int i = 1; i <= initialLength; i++) {
                  final bpRef = databaseReference.child(
                      'users/' + uid + '/goal/water_intake/' + i.toString());
                  bpRef.remove();
                }
                for (int i = 0; i < waterintake_list.length; i++) {
                  ...
                  bpRef.set({
                    ...
                    "timeCreated":
                        "${waterintake_list[i].timeCreated.hour.toString().padLeft(2, "0")}:${waterintake_list[i].timeCreated.minute.toString().padLeft(2, "0")}"
                  });
                }
                Navigator.of(context).pop();
              },
