...
class sleep_doctor_view extends StatefulWidget {
  ...
  sleep_doctor_view({Key key, this.userUID, this.fitbitToken}): super(key: key);
  @override
  _sleep_doctor_viewState createState() => _sleep_doctor_viewState();
}

class _sleep_doctor_viewState extends State<sleep_doctor_view> {
  ...
  void addtoRecommendation(String message, String title, String priority, String redirect){
    ...
    final notifref = databaseReference.child('users/' + userUID + '/recommendations/');
    getRecomm();
    notifref.once().then((DataSnapshot snapshot) {
      if(snapshot.value == null){
        final notifRef = databaseReference.child('users/' + userUID + '/recommendations/' + 0.toString());
        ...
        final notifRef = databaseReference.child('users/' + userUID + '/recommendations/' + (recommList.length--).toString());
  ...
  void getRecomm() {
    ...
    final readBP = databaseReference.child('users/' + userUID + '/recommendations/');
  ...
  void initNotif() {
    ...
    final User user = auth.currentUser;
    final userUID = widget.userUID;
    final readProfile = databaseReference.child('users/' + userUID + '/personal_info/');
